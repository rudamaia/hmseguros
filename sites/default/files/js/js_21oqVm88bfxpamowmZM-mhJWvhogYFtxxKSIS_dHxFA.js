...
jQuery(document).ready(function() {
  jQuery('.path-node .field--name-body p > img').parent().addClass('img-mobile');
  jQuery('a:contains("Cotação")').replaceWith('<a class="typeform-share link" href="https://hmseguros.typeform.com/to/CKWlHC" data-mode="2" target="_blank">Cotação</a>');
   jQuery('a:contains("Faça uma cotação")').replaceWith('<a class="typeform-share btn btn-danger btn-lg link" href="https://hmseguros.typeform.com/to/CKWlHC" data-mode="2" target="_blank">Faça uma cotação</a>');
  jQuery('.produto-destaque').css("background", "url(" + jQuery('.produto-destaque img').attr('src') + ")");
  jQuery('.produto-destaque img').remove();
  jQuery('#contact-message-cotacao-form .form-submit').text('Solicitar Cotação');
  jQuery('#backtotop').each(function(){
		...
		jQuery(window).scroll(function () {
			if (jQuery(window).scrollTop() > 400) {
				  jQuery( "#backtotop" ).addClass('shown');
			}
			if (jQuery(window).scrollTop() < 400) {
				  jQuery( "#backtotop" ).removeClass('shown');
			}
		});
  if (jQuery(window).width() > 768 && jQuery('body').hasClass('page-node-type-page')) {
	      jQuery('#navbar').removeClass('container');
  }
  if (jQuery(window).width() > 768 && jQuery('body').hasClass('page-node-type-produto')) {
	      jQuery('#navbar').removeClass('container');
	      jQuery('#block-views-block-produto-destaque-block-1').insertAfter('#navbar');
  }
  jQuery(window).scroll(function () {
     if (jQuery(window).scrollTop() > 350 && jQuery('body').hasClass('path-frontpage')) {
 ...
 var $contactForm = jQuery('#contact-message-contato-form, #contact-message-cotacao-form');
		$contactForm.submit(function(e) {

  		var validation;
          jQuery('.form-item .required').each(
            function () {
              if (jQuery(this).val() === '') {
                validation = 0;
              }
          ...
          if (validation === 0) {
			...
			jQuery.ajax({
				url: '//formspree.io/henrique@hmseguros.com.br',
				method: 'POST',
				type: 'POST',
				...
				success: function(data) {
					...
					jQuery('#edit-submit, .webform-submit').prop('disabled', true);
				},
				error: function(err) {
					$contactForm.find('.alert--warning').hide();
